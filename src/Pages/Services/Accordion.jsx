...
const Accordion = () => {
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-violet-50 dark:bg-neutral-900 rounded-lg shadow-md mt-10">
      {items.map((item, index) => (
        <div key={index} data-aos={index % 2 === 0 ? "fade-left" : "fade-right"} data-aos-delay="500" className=" border-b border-violet-200 dark:border-neutral-700 last:border-0">
          <button
          ...
          >
            {item.title}
            <span className="text-xl ms-2">{openIndex === index ? "−" : "+"}</span>
          </button>
          <div
